...
public class ConsulRegistry implements RegisterService {
    ...
    public void unsubscribe(URL url, EventListener eventListener) {
        ServiceHealthCache serviceHealthCache = cacheMap.get(url);
        if (serviceHealthCache != null) {
            List<Map<EventListener, ConsulCache.Listener<ServiceHealthKey, ServiceHealth>>> maps = listenerMap.get(serviceHealthCache);
            List<Map<EventListener, ConsulCache.Listener<ServiceHealthKey, ServiceHealth>>> list = maps.stream().filter(m -> m.get(eventListener) != null).collect(Collectors.toList());
