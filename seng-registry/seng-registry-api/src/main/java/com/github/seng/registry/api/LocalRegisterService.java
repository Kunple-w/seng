...
public class LocalRegisterService implements RegisterService {
    ...
    protected Map<URL, Set<com.github.seng.registry.api.EventListener>> listenerMap = new HashMap<>();
    ...
    private class NotifyWorker implements Runnable {
        ...
        public void run() {
            ...
            Set<com.github.seng.registry.api.EventListener> listeners = listenerMap.get(url);
            if (listeners != null && !listeners.isEmpty()) {
                for (com.github.seng.registry.api.EventListener listener : listeners) {
...
//        private Set<URL> old;
    ...
    @Override
    public void subscribe(URL url, com.github.seng.registry.api.EventListener eventListener) {
        if (url == null) {
            throw new IllegalArgumentException("serviceName is null");
        }
        List<URL> urls = lookup(url);
        if (urls.isEmpty()) {
            throw new SengRuntimeException(url + "not register");
        }

        listenerMap.compute(url, (k, v) -> {
            if (v == null) {
                Set<com.github.seng.registry.api.EventListener> newSet = new HashSet<>();
